...
@Log
public class OV_Issue_DataSource extends ListUtils {

	private List<OV_Issue> list = null;
	SimpleRedmineConnector con = new SimpleRedmineConnector();

	public OV_Issue_DataSource(Class clazz) {
	...
	public void load(boolean testMode) {
		...
		if (testMode) {
			...
			try {
				List<Issue> list2 = con.redmine.getIssueManager().getIssues("vepg-si", 125);// 2018ALL
